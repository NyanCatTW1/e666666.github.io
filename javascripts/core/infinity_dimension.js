...
function getIDdiv(){
  ...
  if (player.galacticSacrifice.upgrades.includes(42)) div *= 1 + Math.log10(player.infinityPower.plus(1).log10()+1)
  return div
}
...
function buyManyInfinityDimension(tier) {
  ...
  dim.baseAmount += 10
  dim.cost = new Decimal(infBaseCost[tier]).times(Decimal.pow(infCostMults[tier]/getIDdiv(), (dim.baseAmount/10 + 1)*(ECTimesCompleted("eterc12")?1-ECTimesCompleted("eterc12")*0.008:1)))
...
function buyMaxInfDims(tier) {
  ...
  let costMult = Math.pow(infCostMults[tier]/getIDdiv(), ECTimesCompleted("eterc12")?1-ECTimesCompleted("eterc12")*0.008:1);
